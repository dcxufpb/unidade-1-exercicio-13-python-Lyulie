...
class ItemVenda:
  ...
  def valor_total(self) -> float:
    porcentagem = 1 - self.desconto
    return (self.quantidade * self.produto.valor_unitario) * porcentagem

  def dados_item(self) -> str:
    porcentagem = 1 - self.desconto
    output = f"{self.item} {self.produto.codigo} {self.produto.descricao} "
    output += f"{self.quantidade} {self.produto.unidade} {(self.produto.valor_unitario * porcentagem):.2f} "
